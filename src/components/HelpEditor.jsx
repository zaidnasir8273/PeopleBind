...
// Shared category + article CRUD UI for both PeopleBind's own global
// documentation (companyId null) and a company's private documentation
// (companyId set). Every query/insert is scoped by companyId so the two
// call sites can never see or write each other's rows -- RLS enforces
// this too, but scoping the client queries keeps each surface showing
// only what it should even before the round trip.
export function HelpEditor({ companyId }) {
  ...
  const categoryFilter = useCallback((query) => (companyId ? query.eq('company_id', companyId) : query.is('company_id', null)), [companyId])

  const loadCategories = useCallback(async () => {
    const { data } = await categoryFilter(supabase.from('help_categories').select('id, name, sort_order')).order('sort_order').order('name')
    setCategories(data ?? [])
    setSelectedCategoryId((prev) => (data ?? []).some((c) => c.id === prev) ? prev : (data?.[0]?.id ?? null))
  }, [categoryFilter])
  ...
  async function addCategory() {
    ...
    const { data, error: saveError } = await supabase.from('help_categories').insert({ name: newCategoryName.trim(), created_by: profile.id, company_id: companyId ?? null }).select().single()
  ...
  async function handleSubmit(e) {
    ...
    const { error: saveError } = editingId
      ...
      : await supabase.from('help_articles').insert({ ...payload, created_by: profile.id, company_id: companyId ?? null })
